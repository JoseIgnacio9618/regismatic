...
export class RegisterEmployeePage implements OnInit, OnDestroy {
  ...
  async submit(): Promise<void> {
    if (this.password !== this.confirmPassword) {
      await this.showError(this.i18nService.t("register_employee.password_mismatch"));
      return;
    }
    ...
    try {
      await this.authService.registerEmployee({
        ...
        inviteCode: this.inviteCode.trim() || undefined,
        requestMessage: this.requestMessage.trim() || undefined
      });
      ...
      await this.showError(error instanceof Error ? error.message : this.i18nService.t("register_employee.error_failed"));
    } finally {
      this.loading = false;
    }
  }
